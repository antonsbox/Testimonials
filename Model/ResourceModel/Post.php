...
/*
 * TestTestimonials module mysql resource
 */
...
class Post extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb {
    ...
    protected function _construct() {
        $this->_init('test_testimonials', 'post_id');
    }
...
//        );
        ...
        if (!is_null($isActive)) {
            $select->where('tp.is_active = ?', $isActive);
        }
    ...
    public function checkUrlKey($url_key) {
        ...
        $select->reset(\Zend_Db_Select::COLUMNS)->columns('tp.post_id')->limit(1);
